...
SELECT *
FROM (
         SELECT ITEM, LOCATION, SEGMENT_ID, PREDICTION_STATUS, HISTORY,
                --   SEASONAL_FORECAST, SEASONAL_ERROR,
                ARIMA_FORECAST, ARIMA_ERROR,
                --   XGBOOST_FORECAST, XGBOOST_ERROR,
                --   MA_FORECAST, MA_ERROR,
                --   BLENDED_FORECAST, BLENDED_ERROR,
                DEMANTRA_FORECAST, DEMANTRA_ERROR
         FROM FORECAST_DATA
         WHERE SEGMENT_ID IN (:segment_id)
         ORDER BY 7 DESC -- The Error Column (Make sure you have only one model)
     )
LIMIT 20;